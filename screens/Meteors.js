...
getMeteors = () => {
    axios
        .get("https://api.nasa.gov/neo/rest/v1/feed?api_key=OTof13eMAxsohOtO9i19HcjoRN9GN3lIQV80Awlu")
        .then(response=>{
...
render() {
        if (Object.keys(this.state.meteor).length === 0) {
            ...
            let meteor_arr = Object.keys(this.state.meteor).map(meteor__date =>{
             return this.state.meteors[meteor__date]
            })
